import pygame # Import pygame library for handling for handling audio playback 
...
# initialize pygame and mixer once
pygame.init()
try:
    ...
    print(" pygame.mixer failed to initialize:", e)
    # continue — later playback will fail but we can handle gracefully
...
async def TextToAudioFile(text, file_path):  # file_path ab argument hai
    communicate = edge_tts.Communicate(text,AssistantVoice, pitch='+5Hz', rate='+13%')
...
#                 # DO NOT call pygame.mixer.quit() here
#             except Exception as e:      -----------------------> 
#                 print(f"Error in finally block: {e}")
...
def TTS(Text, func=lambda r=None: True):
    file_path = f"Data/speech_{uuid.uuid4().hex}.mp3"  # Naya unique filename har call par
    os.makedirs(os.path.dirname(file_path), exist_ok=True)

    try:
        asyncio.run(TextToAudioFile(Text, file_path))  # Unique file_path bhejo
