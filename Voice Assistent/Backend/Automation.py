...
import subprocess # Import subprocess for intreacting with the system. 
...
SystemChatBot = [{"role": "system", "content": f"Hello, I am {os.environ['Username']}, You're a content writer. Write clean, concise letters and applications."}]
...
def Content(Topic):
    ...
    #  Nested function to gererate content using the  AI chatbot. 
    def ContentWriterAI(prompt):
        messages.append({"role": "user","content": f"{prompt}"})  # Add the user's  prompt to messages. 

        completion = client.chat.completions.create(
            model = "llama-3.3-70b-versatile", # Specify the AI model.
            messages = SystemChatBot + messages, # Include system insructions and chat history. 
...
# Content("aaplication for a sick leave")
...
#         print(f"App not found: {e}. Trying web search...")
#         def extract_links(html):
#             if html is None:
#                 return []
...
#             webopen(links[0])
#         else:
#             # Agar link nahi milta toh direct Google search page khol do
...
#     # Aur bhi apps ka URL yahan dal sakte ho
# }

# def OpenApp(app, sess=None):  # sess argument ignore kar sakte ho agar web search nahi karna
#     app_lower = app.lower()
#     try:
#         # Agar PC mein app installed hai toh yahan apni open code logic rakh sakte ho
#         appopen(app, match_closest=True, output=True, throw_error=True)
#         return True
#     except Exception as e:
#         print(f"App not found: {e}. Opening website if available...")
#         if app_lower in app_websites:
#             webbrowser.open(app_websites[app_lower])
#         else:
#             # Agar mapping nahi milti toh Google search kholna
#             webbrowser.open(f"https://www.google.com/search?q={app}")
        # return True

def OpenApp(app):
    simple_app = app.lower().replace(' ', '')
    url = f"https://www.{simple_app}.com"
    try:
        # Sabse pehle app ko open karne ki koshish karo (agar install hai)
        appopen(app, match_closest=True, output=True, throw_error=True)
        return True
    except Exception:
        # Agar fail ho ya app installed nahi ho toh direct website open karo
        webbrowser.open(url)
        return True
...
if __name__ == "__main__":
    playonyt("yariyaan")
    # playonyt("apne hi banai ye halat meri by guddi gilahari")
    # asyncio.run(Automation(["open facebook", "open instagram","open telegram","play badboy  from street dancer","content song for me",]))
